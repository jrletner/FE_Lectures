...
app.get("/jokes", (req, res) => {
    ...
    res.status(200).json(jokes[ranIndex]);
})

// 3. GET a single joke by id
...
app.get("/:type", (req, res) => {
    ...
    const typeJokes = jokes.filter((j) => j.jokeType.toLowerCase() === jokeType)
    res.json(typeJokes)
})
...
app.put("/jokes/:id", (req, res) => {
    const { jokeText, jokeType } = req.body;
    const id = Number(req.params.id);
    if (!jokeText) return res.send("WTF?")
    const updatedJoke = {
        id,
        jokeText,
        jokeType
    }
    const foundJokeIndex = jokes.findIndex((j) => j.id === id);
...
app.patch("/jokes/:id", (req, res) => {
    ...
    const updatedJoke = {
        id,
        jokeText: jokeText || foundJoke.jokeText,
        jokeType: jokeType || foundJoke.jokeType
    }
    const foundJokeIndex = jokes.findIndex((j) => j.id === id)
    jokes[foundJokeIndex] = updatedJoke
    res.json(updatedJoke)
...
app.delete("/jokes/:id", (req, res) => {
    const id = Number(req.params.id);
    const jokeExists = jokes.some((j) => j.id === id);
    if (!jokeExists) return res.status(404).send("Joke not found")
