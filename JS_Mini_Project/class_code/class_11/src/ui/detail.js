...
export function renderClubDetail(club) {
  ...
  const card = document.createElement('div'); card.className = 'card'; card.dataset.clubId = club.id;
  ...
  const eventsHtml = club.events.map(evt => {
    const when = evt.friendlyWhen; const pastBadge = evt.isPast ? '<span class="badge">Past</span>' : '';
    return `<li>
  ...
  card.innerHTML = `
    ...
    <div class="event-section">
      ...
      <ul class="event-list">${eventsHtml || '<li><em>No events yet</em></li>'}</ul>
      <div class="inline-form">
