...
export function renderClubs(visibleClubs) {
  ...
  container.innerHTML = '';
  if (visibleClubs.length === 0) { const p = document.createElement('p'); p.textContent = 'No clubs match your filters.'; container.appendChild(p); return; }
  visibleClubs.forEach((club) => {
    const card = document.createElement('div'); card.className = 'club-card'; card.dataset.clubId = club.id;
...
export function setStatus(clubId, message) { const el = document.getElementById(`status-${clubId}`); if (el) el.textContent = message; }
