# Class 7 — Delta Walkthrough (only new/changed snippets)

This guide shows only what changed from Class 6 to Class 7. Unmentioned code stays the same.

---

## app.js — NEW utilities

Place near the top, after the ID helper.

```js
// debounce: delay calling fn until there are no new calls for `delay` ms
...
## app.js — CHANGED: list derivation via transforms + pipe

Replace your previous `getVisibleClubs` implementation with these transforms and the composed function.
...
const applyOnlyOpen = (list) =>
  ui.onlyOpen ? list.filter((c) => c.seatsLeft > 0) : list;
...
## app.js — NEW: debounced search wiring

Replace the search input listener with a debounced handler.
...
## index.html — Optional page copy update

Only if you want the Class 7 text in the tab title and subtitle.
...
No changes from Class 6.

---

## Appendix: Completed files for Class 7
