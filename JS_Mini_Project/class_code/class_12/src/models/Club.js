...
export class Club {
  ...
  toPlain() { return { id: this.id, name: this.name, capacity: this.capacity, members: this.members.map(m => m.toPlain()), events: this.events.map(e => e.toPlain()) }; }
  static fromPlain(obj) {
    ...
    if (!obj.members && typeof obj.current === "number") { for (let i = 0; i < obj.current; i++) c.addMember(`Member ${i + 1}`); }
    c.sortEvents();
    return c;
  }
}
